...
# 1. 생성 (create) 
    async def create_repository(self, comment): # 새로운 comment 생성 
        comments = self.collection.insert_one(comment) # mongodb 명령어로 새로운 comment의 data 등록
                    # 단일 문서로 comment 생성 
...
# 5. 일부 조회 (read) - comment id
    async def read_repository_commentid(self, comment_id: ObjectId): # comment id로 일부 comment의 data 조회
            comment = self.collection.find_one({"_id":comment_id}) # mongodb 명령어로 ObjectId로 지정한 comment id로 comment 조회 
            if comment:     
...
# 4. 전체 조회 (raed) 
    async def read_repository(self) -> list: # list로 comment 결과값 반환
        ...
        comments = self.collection.find({}) # mongodb명령어로 comment의 data 전체 조회
...
# 3. 삭제 (delete)
    async def delete_repository(self, comment_id: ObjectId) -> bool: # comment id로 comment 삭제 
        comments = self.collection.delete_one({"_id":comment_id}) # comment 삭제
